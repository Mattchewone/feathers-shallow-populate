...
- `catchOnError` (_optional_) Continues populating, if an error occurs (e.g. because of missing authentication). Also can be set per `include` individually
...
| `catchOnError`   | Continues populating, if an error occurs (e.g. because of missing authentication). Also can be set on the prior options<br><br>**optional - default:** `false`<br>**Type:**: `{Boolean}` |
