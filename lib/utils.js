...
const chainedParams = async (paramsArr, context, item) => {
  ...
  for (let i = 0, n = paramsArr.length; i < n; i++) {
    ...
    if (_isFunction(params)) {
      params = (item == null)
        ? params(resultingParams, context)
        : params.call(item, resultingParams, context)
      params = await Promise.resolve(params)
    }
    if (params && params !== resultingParams) _merge(resultingParams, params)
  }
