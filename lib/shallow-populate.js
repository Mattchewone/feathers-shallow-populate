...
module.exports = function (options) {
  ...
  const includesByKeyHere = includes.reduce((includes, include) => {
  ...
  return async function shallowPopulate (context) {
    ...
    const requests = includes.map(async include => {
      ...
      let providedParams = (_isFunction(include.params)) ? include.params(params, context) : include.params

      providedParams = await Promise.resolve(providedParams)

      if (providedParams && params !== providedParams) params = _merge(params, providedParams)
    ...
    responses.forEach((response, index) => {
      ...
      data.forEach(item => {
        ...
        if (keyHere) {
          if (Array.isArray(keyHere)) {
            if (!include.asArray) {
              _set(item, include.nameAs, getRelatedItems(keyHere[0], relatedItems, include))
            } else {
              _set(item, include.nameAs, getRelatedItems(keyHere, relatedItems, include))
            }
          } else {
            _set(item, include.nameAs, getRelatedItems(keyHere, relatedItems, include))
          }
