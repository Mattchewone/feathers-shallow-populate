...
module.exports = function (options) {
  ...
  return async function shallowPopulate (context) {
    ...
    const cumulatedRequests = cumulatedIncludes.map(async include => {
      ...
      originalParamsArray.push(params)
      ...
      const result = await app.service(service).find(Object.assign({}, params, { query }))
    ...
    cumulatedResponses.forEach((response, index) => {
      ...
      const { nameAs, keyThere } = include

      const params = originalParamsArray[index]
