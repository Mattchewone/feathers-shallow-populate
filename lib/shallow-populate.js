        ...
        if (Array.isArray(current[key])) {
          ...
          if (keyHere) {
            if (Array.isArray(keyHere)) {
              keyHere.forEach(hereKey => {
                byKeyHere[key][hereKey] = byKeyHere[key][hereKey] || []
                byKeyHere[key][hereKey].push(current)
              })
            } else {
            ...
            includeIds
              ...
              .forEach(includeId => {
                ...
                items.forEach((curItem, index) => {
                  const newItem = getByDot(curItem, include.nameAs) || []
                  newItem.push(item)
                  setByDot(curItem, include.nameAs, newItem)
                })
              })
          })
        })
